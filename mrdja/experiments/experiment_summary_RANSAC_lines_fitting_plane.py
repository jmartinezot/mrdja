...
def compute_parameters_RANSAC_line (line_iterations):
    ...
    number_lines_pairs = number_chosen_lines * (number_chosen_lines - 1) / 2
    ...
    # for num_iterations in [10, 11, 12]:
        ...
        for j in range(10):
            ...
            for i in range(len(list_sse_05)):
                ...
                how_many, inliers = coreransac.get_how_many_below_threshold_between_plane_and_points_and_their_indices(np_points, new_plane, threshold)
            ...
            dict_standard_RANSAC_results = {"n_points_pcd": len(np_points), "n_inliers_maximum": str(len(inliers)), "best_plane": plane_model, "threshold": threshold}
            dict_line_RANSAC_results = {"n_points_pcd": len(np_points), "n_inliers_maximum": str(how_many_maximum), "best_plane": best_plane, "threshold": threshold}
