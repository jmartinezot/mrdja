...
def get_lines_from_number_inliers(RANSAC_data_from_file, number_inliers):
    lines = []
    for iteration_number in range(len(number_inliers)):
        if number_inliers[iteration_number] > 0:
            dict_iteration_results = RANSAC_data_from_file["ransac_iterations_results"][iteration_number]
            lines.append(dict_iteration_results["current_line"])
    return lines

# create a function that, when ordering the number_inliers, also orders the curr
